...
export async function seedLeague(db: Db) {
  ...
  // Chosen so the settled round exercises both articles of the Constitution at once:
  // `t2` and `t3` each named `t1`, who went on to win it, so both owe an apology — and
  // `t1` named `t3`, who finished the round last, which with `t1`'s own win is the whole
  // of article 4 and earns them the right to denigrate `t3`. So `t3` collects both marks
  // from one round. It also leaves `t3` alone at the top of the most-hated board.
  await db.insert(necroporraVotes).values([
    { gameweek: SEED_SETTLED_WEEK, teamId: "t1", firstTeamId: "t3", secondTeamId: null },
