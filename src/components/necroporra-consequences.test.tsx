...
describe("NecroporraConsequences", () => {
  it("writes both rules down, so nobody has to remember them", () => {
    const html = render({ apologists: [], hateTarget: null, winnerTeamIds: [] });
  ...
  it("names the apologist and the winner they picked, both by name", () => {
    const html = render({ apologists: ["b"], hateTarget: null, winnerTeamIds: ["a"] });
  ...
  it("names every apologist when several called it wrong", () => {
    const html = render({ apologists: ["b", "d"], hateTarget: null, winnerTeamIds: ["a"] });
    expect(html).toContain(
  ...
  it("says who sends the hate message and who receives it", () => {
    const html = render({ apologists: ["d"], hateTarget: "d", winnerTeamIds: ["a"] });
  ...
  it("says nobody named the winner rather than leaving a gap", () => {
    ...
    const html = render({ apologists: [], hateTarget: null, winnerTeamIds: ["a"] });
  ...
  it("falls back to the id rather than printing an empty name", () => {
    const html = render({ apologists: ["gone"], hateTarget: null, winnerTeamIds: ["a"] });
  ...
  it("names every co-leader, because the owner ruled they count equally", () => {
    ...
    const html = render({ apologists: ["c"], hateTarget: null, winnerTeamIds: ["a", "b"] });
    expect(html).toContain(
  ...
  it("has both co-leaders send the hate message", () => {
    const html = render({ apologists: ["c"], hateTarget: "c", winnerTeamIds: ["a", "b"] });
